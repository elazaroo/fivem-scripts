...
RegisterNetEvent("keko_dispatch:Client:SendAlert")
AddEventHandler("keko_dispatch:Client:SendAlert", function(title, text, coords, panic, id)
    Numcall = Numcall + 1
    Allcalls = Allcalls + 1
...
RegisterNetEvent("keko_dispatch:Client:SendVehRob")
AddEventHandler("keko_dispatch:Client:SendVehRob", function(coords, model, plate, color, id)
    Numcall = Numcall + 1
    Allcalls = Allcalls + 1
...
RegisterNetEvent('keko_dispatch:Client:Cooldown')
AddEventHandler('keko_dispatch:Client:Cooldown', function()
    Cooldown = true
        ...
        if Config.ShootingAlerts and IsPedShooting(ped) then
            for k, v in pairs(Config.Jobs) do
                ...
                if Cooldown == false then
                    TriggerServerEvent('keko_dispatch:Server:SendAlert', job, title, text, coords, panic, id)
                    ShootingBlip()
                    TriggerEvent('keko_dispatch:Client:Cooldown')
                end
            end
        end
        Wait(1)
...
RegisterNUICallback('updateUserUnit', function(data)
    TriggerServerEvent('keko_dispatch:Server:updateUserUnit', GetPlayerServerId(PlayerId()), data.type, data.value)
end)
