...
if Config.Framework == 'ESX' then
    ...
    AddEventHandler("esx:playerLoaded", function(xPlayer)
        ...
        if ESX.PlayerData.job.name == Config.DispatcherJob then
            ...
            TriggerServerEvent('keko_dispatch:Server:addGlobalUnit', user, name, number,
    ...
    UpdateUserData = function()
        ESX.TriggerServerCallback('keko_dispatch:getGlobalUnits', function(units)
            SendNUIMessage({
                type = 'updateUserData',
                units = units,
            });
    ...
    RegisterCommand(Config.VehicleRob.command, function(args)
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            ESX.ShowNotification(LC['Alert_Sent'])
            TriggerServerEvent("keko_dispatch:Server:SendVehRob", 'police', coords, model, plate, color, id)
        else
...
if Config.Framework == "QBCore" then
    ...
    UpdateUserData = function()
        QBCore.Functions.TriggerCallback('keko_dispatch:getGlobalUnits', function(cb)
            SendNUIMessage({
                type = 'updateUserData',
                units = cb,
            });
...
RegisterKeyMapping('alertas', 'Menu alertas', 'keyboard', 'f4')
RegisterKeyMapping('panic', 'Enviar alerta de panico', 'keyboard', 'f9')