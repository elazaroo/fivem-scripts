...
Ui.Clear = function (data) {
    ...
    $(".dp-alert-cont h1").text('Sin Alertas');
    $(".dp-alert-cont p").text('No hay alertas');
    $(".page-cont").removeClass('panic');
    $(".page-cont").text(0 + '/' + 0);
    $('.dp-alert-color').hide()
    $(".dp-alert-cont h3").text('Actualmente no tienes alertas');
...
Ui.UpdateServerSideData = function (type, value) {
    let data = { type, value }
    $.post('https://keko-dispatch/updateUserUnit', JSON.stringify(data));
}
...
$(document).on('keydown', function () {
    switch (event.keyCode) {
        case 27:
            $.post(`https://keko-dispatch/closeLarge`);
            break;
    }
});